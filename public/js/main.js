...
function uninvite(who) { // Uninvite someone
	var payload = {};
	payload.requested_user = who;
...
function game_start(who) { // Game start message to the server
	var payload = {};
	payload.requested_user = who;
...
socket.on('game_start_response', function(payload){ // engageg
	if (payload.result === 'fail'){
		alert(payload.message);
		return;
	}
...
socket.on('game_update',function(payload){
	...
	if('undefined' == typeof board || !board){
		console.log('Internal error: recevied a malformed board update from the server');
		return;z
	}
...
socket.on('play_token_response',function(payload){
	...
	/* Check for a good_token_response update*/
	if (payload.result == 'fail'){
		console.log(payload.message);
		alert(payload.message);
		return;
